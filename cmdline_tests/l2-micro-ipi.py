...
def l2_micro_ipi(iovirt, posted, level, mi, mi_role):
    boot_l1(iovirt, posted, level, mi)
    pin_vcpus_l1()

    micro_cmd = "cd kvm-unit-test && ./run-micro.sh"
    child.sendline(micro_cmd)
    child.interact()
...
child.sendline('')
wait_for_prompt(child, hostname)
l2_micro_ipi(iovirt, posted, level, mi, mi_role)

child.sendline('service apache2 start')
child.expect('L2.*$')
