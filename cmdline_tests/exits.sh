...
function save_prev {
	...
	PREV_HANDLE_EXIT[$1]=$(ssh $3@$2 'sudo cat /sys/kernel/debug/kvm/abc_exits')
...
function save_curr {
	...
	CURR_HANDLE_EXIT[$1]=$(ssh $3@$2 'sudo cat /sys/kernel/debug/kvm/abc_exits')
	CURR_MSR_EXIT[$1]=$(ssh $3@$2 'sudo cat /sys/kernel/debug/kvm/msr')
}

function print_title {
	echo "total,io,irq,halt,mmio,handle_exits,msr" >> $ALL_EXITS
}
...
function save_diff {
	...
	N_HANDLE_EXIT=$((CURR_HANDLE_EXIT[$1] - PREV_HANDLE_EXIT[$1]))
	N_MSR_EXIT=$((CURR_MSR_EXIT[$1] - PREV_MSR_EXIT[$1]))
	N_EXIT=$((CURR_EXIT[$1] - PREV_EXIT[$1]))
	echo "Number of exits: $N_EXIT"

	echo $N_EXIT,$N_IO_EXIT,$N_IRQ_EXIT,$N_HALT_EXIT,$N_MMIO_EXIT,$N_HANDLE_EXIT,$N_MSR_EXIT >>$ALL_EXITS
}
